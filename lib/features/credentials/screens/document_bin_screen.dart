...
class DocumentBinScreen extends StatefulWidget {
  const DocumentBinScreen({Key? key}) : super(key: key);

  @override
  State<DocumentBinScreen> createState() => _DocumentBinScreenState();
}

class _DocumentBinScreenState extends State<DocumentBinScreen> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _filteredDocuments = _deletedDocuments;
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  void _filterDocuments(String query) {
    setState(() {
      _filteredDocuments = _deletedDocuments.where((document) {
  ...
  Future<void> _restoreFromBin(Document document) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to restore document: ${e.toString()}'),
            backgroundColor: Colors.red,
          ),
        );
      }
                                ...
                                return Padding(
                                  ...
                                  child: Slidable(
                                    ...
                                    child: DocumentCard(
                                      document: document,
                                      showDeleted: true,
                                      onRestore: () => _restoreFromBin(document),
                                      onDelete: () => _permanentlyDelete(document),
